...
public class PageVo {
	...
	//인자있는 생성자를 선언하게 되면 default생성자(인자가 없는 생성자)가 생성이 안됨
	//대다수의 framework에서는 인자가 없는 생성자를 요구하는 경우가 많음
	//framework에서는 해당 클래스의 인스턴스를 생성해야하는데 인자가 있으면 어떻게 만들어야하는지 알수가 없는 경우가 대부분
	...
	public PageVo(int page, int pageSize, int boardmenu_seq){
		this.page = page;
