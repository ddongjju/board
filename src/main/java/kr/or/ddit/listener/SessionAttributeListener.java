...
public class SessionAttributeListener implements HttpSessionAttributeListener{
	...
	public void attributeAdded(HttpSessionBindingEvent event) {
		if("S_MEMBER".equals(event.getName())) {
			...
			logger.debug("사용자 로그인 : {}",memberVo.getUser_id());
					
			userMap.put(memberVo.getUser_id(), memberVo);
	...
	public void attributeRemoved(HttpSessionBindingEvent event) {
		if ("S_MEMBER".equals(event.getName())) {
			...
			MemberVo memberVo = (MemberVo) event.getValue();
			logger.debug("사용자 로그아웃 : {}", memberVo.getUser_id());

			userMap.remove(memberVo.getUser_id());
